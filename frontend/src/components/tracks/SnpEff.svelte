...
const showModal = (position) => {
    ...
    eventbus.emit('modal:open', {
        component: 'SnpEffAnnotation',
        props: {snpeff_data: snpeff_data, position: position}
    });
};
...
onMount(async () => {
    //if (tippyInstancesSnpeff !== undefined && typeof tippyInstancesSnpeff[0].destroy === "function") { tippyInstancesSnpeff[0].destroy(); }
    tippyInstancesSnpeff = delegate(rootElem.querySelector('#tracks-container'), tippyProps);
});
...
<div class="track snpeff" style="height: 23px;"><div class="label">SnpEff annotations</div>
    ...
    <span on:click|preventDefault={showModal(position)} class="variant-hover snpeff-indicator" data-tippy-content="SnpEff annotation" data-position="{position}" style="width: {$variantWidth}px;">
